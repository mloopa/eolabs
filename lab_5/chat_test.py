...
class HyperspectralCube:
    ...
    def read_rgb(self, r_idx=None, g_idx=None, b_idx=None) -> np.ndarray:
        ...
        # Use GLOBAL percentiles (shared across RGB)
        valid = ~np.isnan(stack)
        if not valid.any():
            return np.zeros((self.lines, self.samples, 3), dtype=np.uint8)
        ...
        # Normalize all bands together
        stack = (stack - lo) / (hi - lo)
        stack = np.clip(stack, 0.0, 1.0)

        # Restore no-data pixels to black
        combined_mask = np.any(np.stack(masks, axis=0), axis=0)
        stack[:, combined_mask] = 0.0

        rgb = (stack.transpose(1, 2, 0) * 255).astype(np.uint8)
        return rgb
